...
spam.trend.seasonal <- lm(spam.ts~time.spam + sin(2*pi*time.spam/12) + cos(2*pi*time.spam/12))
...
e.ts.spam <- ts(spam.trend.seasonal$residuals)
...
spam.auto <- auto.arima(spam.ts , approximation = FALSE)
summary(spam.auto)
