...
Smin.fitted <- spam.trend.seasonal$fitted.values + fitted(spam.auto)

ggplot() + geom_line(aes(x=time.spam,y=spam.ts[1:length(time.spam)],color="True")) +
  geom_line(aes(x=time.spam,y=Smin.fitted,color="Fitted")) + xlab("Time") + 
  ylab("Richmond Tmin")
