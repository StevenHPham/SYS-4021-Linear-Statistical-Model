...
# use png format
dir.create(paste(datadir,"/figures",sep="")) #Create a folder "figures" in data folder

setwd(paste(datadir,"/figures", sep="")) 
png('Image1.png',width = 1920,height = 1080)
uva.pairs(spam[,c(1:10,58)])
dev.off()
#Takeaway: v58 graphs are not linear, so dont use linear model, (bottom rows are relationship between predictors v1-v57 and response variable v58 spam or not spam) 
...
dev.off()
#Takeaway: find graphs that have a log fit and high correlation index
...
#Takeaway:  Indicator 1: Difference in median means the distribution is shifted. look for shifts in median 
...
#Takeaway: Perpendicular predictors are independent, indicating one cluster is spam and another not spam
